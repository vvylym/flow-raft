...
mod tests {
    ...
    async fn test_register_workflow() {
        ...
        b.add_node("task1", node(dummy_task), None)
            .set_root("task1");
        let tg = b.build().unwrap();
        let workflow = tg.workflow_def("test").unwrap();
    ...
    async fn test_register_workflows() {
        ...
        let mut b1 = TypedGraphBuilder::new("test1");
        b1.add_node("task1", node(dummy_task), None)
            .set_root("task1");
        let workflow1 = b1.build().unwrap().workflow_def("test1").unwrap();

        let mut b2 = TypedGraphBuilder::new("test2");
        b2.add_node("task2", node(dummy_task), None)
            .set_root("task2");
        let workflow2 = b2.build().unwrap().workflow_def("test2").unwrap();
