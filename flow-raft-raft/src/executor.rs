...
use crate::command::WorkflowCommandBuilder;
...
pub struct WorkflowExecutor {
    ...
    /// Node ID (reserved for future node-specific execution logic)
    ///
    /// # Note
    /// This field is reserved for future implementation of node-specific
    /// execution policies, routing, or node-aware task scheduling.
    #[allow(dead_code)] // Reserved for future node-specific logic
    node_id: u64,
}
...
impl WorkflowExecutor {
    ...
    #[tracing::instrument(level = "debug", skip(self), fields(workflow_id = %workflow_id))]
    pub async fn get_ready_tasks(&self, workflow_id: &WorkflowId) -> Vec<TaskId> {
        ...
        if !matches!(workflow.state, flow_raft_core::WorkflowState::Running) {
            return Vec::new();
        }
        ...
        let in_progress: HashSet<TaskId> = workflow
            ...
            .filter(|(_, exec)| {
                matches!(
                    exec.state,
                    flow_raft_core::TaskState::Running | flow_raft_core::TaskState::Scheduled
                )
            })
            .map(|(id, _)| *id)
            .collect();
        ...
        use flow_raft_core::ready_tasks;
    ...
    #[tracing::instrument(level = "info", skip(self, handler, inputs), fields(workflow_id = %workflow_id, task_id = %task_id))]
...
mod tests {
    ...
    use chrono::Utc;
    use flow_raft_core::{
        TaskDefinition, TaskDependencies, TaskState, WorkflowSnapshot, WorkflowState,
    };
    use indexmap::IndexMap;
    use openraft::Raft;
    ...
    async fn test_get_ready_tasks_workflow_not_running() {
        ...
        let request = crate::types::Request::CreateWorkflow {
        ...
        assert!(
            ready.is_empty(),
            "Should return empty for non-running workflow"
        );
    }
    ...
    async fn test_get_ready_tasks_with_dependencies() {
        ...
        workflow
            .dependencies
            .insert(task1_id, TaskDependencies::default());
        ...
        assert!(
            ...
            "task2 should not be ready until task1 completes"
        );
    }
    ...
    async fn test_get_ready_tasks_excludes_running_tasks() {
        ...
        workflow
            .dependencies
            .insert(task2_id, TaskDependencies::default());
        ...
        assert!(
            ...
            "task1 should not be ready (it's running)"
        );
    ...
    async fn test_execute_task_success() {
        ...
        executor
            .raft
            .initialize([1u64].into_iter().collect::<BTreeSet<_>>())
            .await
            .unwrap();
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
    ...
    async fn test_execute_task_failure() {
        ...
        assert!(
            result.is_ok(),
            "Raft write should succeed even if task fails"
        );
        ...
        match &execution.unwrap().state {
            TaskState::Failed {
                ...
                assert_eq!(*failure_kind, flow_raft_core::FailureKind::Retryable);
            }
