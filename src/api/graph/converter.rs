...
use crate::api::graph::builder::Graph;
use crate::core::{
    RetryConfig, Task, TaskDependencies, Workflow, WorkflowDraft, WorkflowError,
    WorkflowId,
};
...
pub fn graph_to_workflow(
    ...
    for (from_name, edges) in &graph.edges {
        ...
        for edge in edges {
            match edge {
                crate::api::graph::builder::EdgeSpec::Simple(to_name) => {
                ...
                crate::api::graph::builder::EdgeSpec::Conditional { then, otherwise, .. } => {
                ...
                crate::api::graph::builder::EdgeSpec::Split { targets, .. } => {
...
pub fn dynamic_graph_to_workflow(
    graph: crate::api::graph::dynamic::DynamicGraph,
    workflow_id: WorkflowId,
    default_retry_config: RetryConfig,
    inputs: serde_json::Value,
) -> Result<Workflow<WorkflowDraft>, WorkflowError> {
    ...
    for (name, dyn_node) in &graph.nodes {
        let graph_node = crate::api::graph::builder::GraphNode {
            name: name.clone(),
    ...
    for (from_name, dyn_edges) in &graph.edges {
        ...
        for dyn_edge in dyn_edges {
            match dyn_edge {
                crate::api::graph::dynamic::DynamicEdgeSpec::Simple(to_name) => {
                    edges.push(crate::api::graph::builder::EdgeSpec::Simple(to_name.clone()));
                }
                crate::api::graph::dynamic::DynamicEdgeSpec::Conditional {
                    condition,
                    then,
                    otherwise,
                } => {
                    edges.push(crate::api::graph::builder::EdgeSpec::Conditional {
                        condition: condition.clone(),
                        then: then.clone(),
                        otherwise: otherwise.clone(),
                    });
                }
                crate::api::graph::dynamic::DynamicEdgeSpec::Split { split, targets } => {
                    edges.push(crate::api::graph::builder::EdgeSpec::Split {
                        split: split.clone(),
