...
// Re-export types needed for the example
use flow_raft_api::graph::NodeName;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let raft =
        openraft::Raft::new(node_id, config, network, log_store, state_machine.clone()).await?;
    ...
    registry
        .register_handler(
            ...
            Arc::new(OrderValidationHandler) as Arc<dyn TaskHandler>,
        )
        .await;
    registry
        .register_handler(
    ...
    use flow_raft_raft::types::Request;
    use flow_raft_server::handlers::HandlerExecutor;
