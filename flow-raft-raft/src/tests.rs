...
async fn wait_for_leader(node: &FlowRaftNode, timeout: Duration) -> Result<(), String> {
...
async fn wait_for_leader_any(node: &FlowRaftNode, timeout: Duration) -> Result<NodeId, String> {
    let start = std::time::Instant::now();
    while start.elapsed() < timeout {
...
#[cfg(test)]
#[allow(clippy::module_inception)] // Standard Rust testing pattern
mod tests {
    use super::*;
    ...
    async fn test_single_node_setup() {
        ...
        assert!(
            init_result.is_ok(),
            "Failed to initialize single-node cluster"
        );
    ...
    async fn test_single_node_create_workflow() {
        ...
        // Wait for leader
        wait_for_leader(&node, Duration::from_secs(5))
            .await
            .unwrap();
    ...
    async fn test_cluster_setup_4_nodes() {
        ...
        for node in &nodes {
            if wait_for_leader_any(node, Duration::from_secs(10))
                .await
                .is_ok()
            {
                leader_found = true;
    ...
    async fn test_cluster_replication() {
        ...
        // Create 4 nodes
        for node_id in 1..=4 {
            let node = create_node(node_id, config.clone(), network.clone())
                .await
                .unwrap();
            nodes.push(node);
        }
